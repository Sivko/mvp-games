...
export const useUserStore = defineStore('user', () => {
  ...
  const playerInitial = ref<string | null>(null);
  ...
  /**
   * Устанавливает initial текущего игрока
   * @param initial - первая буква имени игрока
   */
  const setPlayerInitial = (initial: string | null) => {
    playerInitial.value = initial;
  };

  /**
   * Глобальная переменная isTelegrammApp - true, если есть player.initial
   */
  const isTelegrammApp = computed(() => {
    return !!playerInitial.value;
  });
