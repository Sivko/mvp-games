...
# Имя архива
ARCHIVE_NAME="admin-image.tar"

# Собираем admin проект
echo "Сборка admin проекта (production mode)..."
cd apps/admin || exit 1
npm run build-production

if [ $? -ne 0 ]; then
  echo "✗ Ошибка при сборке admin проекта"
  exit 1
fi

echo "✓ Admin проект успешно собран"
...
echo "✓ Контейнер admin успешно перезапущен"
